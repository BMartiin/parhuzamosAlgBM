...
//Kep mentese PPM fajlba
void save_image(Color image[HEIGHT][WIDTH], int frame) {
    char filename[64];
    sprintf(filename, "man2_frame_%03d.ppm", frame);
    ...
    for (int frame = 0; frame < 60; frame++) {
        ...
        for (int y = 0; y < HEIGHT; y++) {
            for (int x = 0; x < WIDTH; x++) {
                ...
                // Adaptive iteration limit for the central region
        ...
        zoom *= zoom_factor;
        printf("Completed: %d/60 frames\n", frame + 1);
    }

    // End measuring time
    clock_t end_time = clock();
    double elapsed_time = (double)(end_time - start_time) / CLOCKS_PER_SEC;
    printf("Animation generation completed in %.2f seconds.\n", elapsed_time);
